...
int search_contact_by_phone( char *phone, struct Contact *c, int *io_count ){
	int status = (get_rec_by_non_ndx_key(phone,c,match_contact_phone,io_count));
	if(status == PDS_REC_NOT_FOUND) return CONTACT_FAILURE;
...
int match_contact_phone( void *rec, void *key ){
	struct Contact *temp = rec;
	char *temp2 = key;
	if(!strcmp(temp2,temp->phone)) return 0;
	else return 1;
...
int delete_contact(int contact_id){
	if(delete_rec_by_ndx_key(contact_id)==PDS_SUCCESS) return CONTACT_SUCCESS;
	return CONTACT_FAILURE;
}